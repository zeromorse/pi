...
- `/model` selects a model. Press `Ctrl+L` to open the same selector. `/flash` toggles between the default model and the lightweight flash model configured through `defaultFlashProvider`/`defaultFlashModel` in [Settings](settings.md).
...
- `/new [name]` starts a new session, optionally naming it (same as `/name`).
...
Use `/tree`, `/fork [name]`, or `/clone [name]` when you want to explore another approach without losing existing work; the optional name sets the new session's display name. Use `/compact` to reduce the conversation history sent to the model. See [Sessions and Context](sessions.md) for these workflows.
