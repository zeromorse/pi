...
`newSession()` and `fork()` accept a `name` option that sets the new session's display name at creation time (same as `/name`). The name is applied after `session_start` and before `withSession` runs, so callbacks observe the named session.
