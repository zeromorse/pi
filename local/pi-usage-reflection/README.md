...
定时 pi 使用反思：每天 10:00 由 LaunchAgent `~/Library/LaunchAgents/com.zeromorse.pi-usage-reflection.plist` 触发，`cd` 中性目录 `~/.pi-usage-reflection/` 后 `pi -p` 调用本仓库 `local/pi-usage-reflection/` skill（幂等软链到 `~/.pi/agent/skills/` 使中性目录可发现；空目录无 `.pi/` 资源不触发项目信任弹窗），只复盘近 1 天与近 7 天两个窗口（跳过 skill 默认的 30 天）。
